...
// Return cpu time of a process
time_t _get_proc_cpu_time(process_t *p) {
	lib_printf("_get_proc_cpu_time()\n");
	time_t max_cpu_time = 0;
	thread_t* t = p->threads;
	while (t != NULL) {
		if(t->cpuTime > max_cpu_time) max_cpu_time = t->cpuTime;
		t = t->next;
	}
	return max_cpu_time;
}

process_t* _get_proc_with_higher_cpu_time(process_t * p1, process_t* p2) {
	lib_printf("_get_proc_with_higher_cpu_time()\n");
	time_t cpu_time1 = _get_proc_cpu_time(p1);
	time_t cpu_time2 = _get_proc_cpu_time(p2);
	if (cpu_time1 > cpu_time2)
		return p1;
	return p2;
}

int _get_proc_priority(process_t* p) {
	lib_printf("_get_proc_priority()\n");
	int result = __INT_MAX__;
	thread_t* t = p->threads;
	while (t != NULL) {
		lib_printf("inside the loop\n");
		if(result > t->priorityBase)
			result = t->priorityBase;
		t = t->next;
	}
	return result;
}

process_t* _proc_highest_cpu_time_helper(int pri_min, int pri_max, rbnode_t *root) {
	lib_printf("_proc_highest_cpu_time_helper()\n");
	if(root == NULL) {
		return NULL;
	}
	// Get this node
	process_t *result = lib_treeof(process_t, idlinkage, root);
	// Check priority
	int priority = _get_proc_priority(result);
	if(priority < pri_min || priority > pri_max) {
		result = NULL;
	}
	// Check left subtree
	if (root->left != NULL) {
		process_t *left = _proc_highest_cpu_time_helper(pri_min, pri_max, root->left);
		result = _get_proc_with_higher_cpu_time(result, left);
	}
	// Check right subtree
	if (root->right != NULL) {
		process_t *right = _proc_highest_cpu_time_helper(pri_min, pri_max, root->right);
		result = _get_proc_with_higher_cpu_time(result, right);
	}
	return result;
}

// Return process id with highest cpu time and priority between min and max
void proc_highest_cpu_time(int pri_min, int pri_max, int* pid, unsigned long long* cpu_time) {
	proc_lockSet(&process_common.lock);
	process_t* result = _proc_highest_cpu_time_helper(pri_min, pri_max, process_common.id.root);
	proc_lockClear(&process_common.lock);
	if(result != NULL) {
		*pid = result->id;
		*cpu_time =  _get_proc_cpu_time(result);
	}
}
