...
int threads_schedule(unsigned int n, cpu_context_t *context, void *arg)
{
	...
	if (current != NULL) {
		...
		if (last_time = 0)
			last_time = now;
		time_t time_spent = now - last_time;

		// Update time spent
		if (current->sched_group == SCHED_GROUP_A)
			time_a += time_spent >> 0;
		else if (current->sched_group == SCHED_GROUP_B)
			time_b += time_spent >> 1;
		else if (current->sched_group == SCHED_GROUP_C)
			time_c += time_spent >> 2;

		// Get the group with lowest time spent
		sched_group = SCHED_GROUP_A;
		if (time_b < time_a)
			sched_group = SCHED_GROUP_B;
		if (time_c < time_b)
			sched_group = SCHED_GROUP_C;
	}
