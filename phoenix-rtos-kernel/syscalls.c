 ...
 */
void syscalls_ch_sched_group(void *ustack)
{
	int group;
	GETFROMSTACK(ustack, int, group, 0);
	proc_current()->sched_group = group;
}
...
void syscalls_highest_cpu_time(void *ustack)
{
	int low, high;
	pid_t *pid;
	time_t *cpu_time;
	GETFROMSTACK(ustack, int, low, 0);
	GETFROMSTACK(ustack, int, high, 1);
	GETFROMSTACK(ustack, pid_t *, pid, 2);
	GETFROMSTACK(ustack, time_t *, cpu_time, 3);
	lib_printf("Looking for processes with priority between %d and %d\n", low, high);
	proc_highest_cpu_time(low, high, pid, cpu_time);
}
